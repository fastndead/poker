...
export default function Modal({ className, isVisible, children, title, onClose }: Props) {
  ...
  return (
    overlayTransition((modalOverlaySpring) => (
      <animated.div
        style={{
          ...modalOverlaySpring,
        }}
        className={classNames('fixed top-0 left-0 w-screen h-screen flex justify-center items-center')}
      >
        ...
        <animated.div
          className='fixed top-0 left-0 opacity-70 w-screen h-screen bg-modal-grey z-40 '
        />
      </animated.div>
    ))
  )  
